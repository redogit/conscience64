...
const playIndex=await readFile(new URL('../index.html',import.meta.url),'utf8');
assert.match(html,/Fuzzball Hidden Game · Alpha/);assert.match(html,/noindex,nofollow/);assert.match(html,/humanoid creatures/i);
assert.ok(!playIndex.includes('fuzzball-hidden'),'hidden alpha must remain unlisted from play/index.html');
...
console.log('PASS Fuzzball Hidden Game alpha: deterministic world, humanoid families, hidden/noindex release surface, absent from public Play catalog.');
