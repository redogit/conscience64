...
### Arcade Forge admitted successor

The useful data-only plug-in idea from the retired `play/mmo/` branch has now been reconstructed inside canonical `play/mmo-world/` rather than reviving a second MMO root.

`play/mmo-world/plugin-contract.json` and `plugin-runtime.mjs` define a bounded local recipe format with `choice`, `input`, and `creative` mechanics. The runtime rejects unknown authority-bearing fields, unsupported mechanics, excessive rewards, ambiguous duplicate choices/answers, oversized imports, excessive shelf entries, and corrupt stored shelves. The Forge page uses a no-network CSP and DOM text rendering for imported/player text.
