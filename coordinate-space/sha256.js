...
function sha256Bytes(bytes){
 ...
 const l=bytes.length, bitLen=l*8, withOne=l+1, pad=(64-((withOne+8)%64))%64,total=withOne+pad+8;
 const data=new Uint8Array(total);data.set(bytes);data[l]=0x80;const dv=new DataView(data.buffer);
 dv.setUint32(total-8,Math.floor(bitLen/0x100000000));dv.setUint32(total-4,bitLen>>>0);
 const w=new Uint32Array(64);
 for(let off=0;off<total;off+=64){for(let i=0;i<16;i++)w[i]=dv.getUint32(off+i*4);
