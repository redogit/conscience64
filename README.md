...
> **Public projection — updated 2026-09-25:** GitHub Pages remains an isolated generated projection. It includes the `public-testbed/**` experiment surface **plus one explicitly owner-authorized curated application route: `play/musilanguage/`**. The repository as a whole, the rest of `play/`, private-origin carriers, and other repositories remain outside Pages publication. Musilanguage predecessor pages remain source/history rather than separate public routes.
...
GitHub Pages publishes a **generated curated projection**, not the repository tree. The admitted source families are `public-testbed/testbed.json`, `public-testbed/site/**`, and a seven-file allowlist for the unified `play/musilanguage/` runtime. `tools/build-public-testbed.mjs` reconstructs the projection, records the exact `main` source revision and every projected file hash, and the source-sync workflow commits only those generated bytes to `gh-pages` while preserving the previous `gh-pages` commit as rollback lineage.

The live verifier independently rebuilds the expected projection from the exact source revision, byte-compares it with the `gh-pages` tree, and then checks the network edge. `play/musilanguage/` is positively required; repository routes such as `README.md`, `research/**`, `about/**`, `play/index.html`, and the old Musilanguage `single.html`, `radio.html`, and `word-forge.html` routes are required to remain absent.

`PUBLIC TEST BED + MUSILANGUAGE != WHOLE REPOSITORY` · `ONE PUBLIC APP != DELETION OF HISTORY` · `PUBLIC EXPERIMENT != VERIFIED TRUTH` · `PRIVATE SOURCE MUST NOT PROPAGATE`