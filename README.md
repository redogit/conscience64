...
> **Public test-bed projection — 2026-09-20:** GitHub Pages is authorized only for the generated `public-testbed/**` experimental projection under issue #166. The repository as a whole, private-origin carriers, and other repositories are not authorized for Pages publication. Older route links below remain project/history references unless the narrow test-bed projection explicitly includes them.
...
GitHub Pages publishes a **generated public-testbed projection**, not the repository tree. The source family is limited to `public-testbed/testbed.json` and `public-testbed/site/**`. `tools/build-public-testbed.mjs` reconstructs the projection, records the exact `main` source revision and every projected file hash, and the source-sync workflow commits only those generated bytes to `gh-pages` while preserving the previous `gh-pages` commit as rollback lineage.

The live verifier independently rebuilds the expected projection from the exact source revision, byte-compares it with the `gh-pages` tree, and then checks the network edge. Repository routes such as `README.md`, `research/**`, `play/**`, and `about/**` are required to be absent from the live projection.

`PUBLIC TEST BED != WHOLE REPOSITORY` · `PUBLIC EXPERIMENT != VERIFIED TRUTH` · `PRIVATE SOURCE MUST NOT PROPAGATE`