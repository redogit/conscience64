...
// Source synchronization, deployment observation, and live verification remain separate boundaries.
...
assert.doesNotMatch(workflow, /actions:\s*write/, 'source sync must not claim Actions write authority');
...
assert.ok(deploymentStatus > verify, 'source sync must observe the exact Pages deployment only after gh-pages matches main');
assert.ok(repositoryDispatch > deploymentStatus, 'live verification dispatch must occur only after exact deployment observation');
...
assert.match(workflow, /event_type:"pages-live-verify"/, 'source sync must emit only the bounded live-verification dispatch type');
assert.match(workflow, /expected_sha:\$sha/, 'source sync must carry the exact synchronized SHA');
assert.doesNotMatch(workflow, /python3 -c/, 'deployment status parsing must not use fragile multiline inline Python');
...
assert.doesNotMatch(liveWorkflow, /workflow_run:/, 'live verification must not rely on the rejected workflow-run handoff');
assert.match(
  liveWorkflow,
...
assert.match(liveWorkflow, /EXPECTED_SHA:\s*\$\{\{ github\.event\.client_payload\.expected_sha \}\}/, 'live verifier must receive the deployed SHA');
assert.match(liveWorkflow, /git fetch --depth=1 origin main/, 'live verification must continue fetching current main');
assert.match(liveWorkflow, /test "\$EXPECTED_SHA" = "\$current_main"/, 'repository-dispatched verification must fail closed if main advanced past the deployed SHA');
...
console.log(`PASS Pages boundary contract: exact source sync, exact deployment observation, bounded repository dispatch, federation-trigger coverage, and ${routes.length} canonical public routes`);
